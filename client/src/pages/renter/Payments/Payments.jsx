...
export default function Payments() {
  ...
  const columns = [
    ...
    { field: "ownerPhone", headerName: "Owner Phone", flex: 1 },
    {
      field: "rent",
      headerName: "Rent",
      flex: 1,
  ...
  const fetchPaymentDetails = async () => {
    try {
      ...
      const rowsWithId = data.paymentList.map((item) => ({
        ...
        ownerName: item.ownerId.userName,
        ownerEmail: item.ownerId.email,
        ownerPhone: item.ownerId.mobileNo,
        ...
        >
          ...
          <DialogContent sx={{ padding: "24px", backgroundColor: "#f7f9fc" }}>
            ...
            <Box display="flex" alignItems="center" gap={1} mb={2}>
              ...
              <Typography variant="body1" sx={{ color: "#455a64" }}>
                <strong>Owner Name:</strong> {selectedPayment.ownerName}
              </Typography>
            </Box>
            <Box display="flex" alignItems="center" gap={1} mb={2}>
              <Email sx={{ color: "#1976d2" }} />
              <Typography variant="body1" sx={{ color: "#455a64" }}>
                <strong>Email:</strong> {selectedPayment.ownerEmail}
              </Typography>
            </Box>
            <Box display="flex" alignItems="center" gap={1} mb={2}>
              <Phone sx={{ color: "#1976d2" }} />
              <Typography variant="body1" sx={{ color: "#455a64" }}>
                <strong>Phone:</strong> {selectedPayment.ownerPhone}
              </Typography>
            </Box>
            <Box display="flex" alignItems="center" gap={1} mb={2}>
              <Bed sx={{ color: "#1976d2" }} />
